...
# 🧠 [تعديل أساسي]: تهيئة قيمة حقل الإدخال في الـ Session State قبل قراءته أو رسمه
...
if not df_db_calc.empty:
    df_db_calc["date_only"] = pd.to_datetime(df_db_calc["التاريخ"]).dt.date
...
st.caption(f"تم فتح {completed} من أصل {len(achievements)} إنجاز")
...
with col_graph1:
    ...
    for m in range(1, 13):
        ...
        if not m_data.empty:
            month_labels.append(datetime.date(current_year, m, 1).strftime('%b'))
            month_positions.append(int(m_data['الأسبوع_السنوي'].median()))
    ...
    fig_heatmap.update_layout(height=420, margin=dict(t=40, b=10, l=5, r=5), xaxis=dict(showgrid=False, side='top', tickvals=month_positions, ticktext=month_labels), yaxis=dict(showgrid=False, autorange='reversed'))
...
st.markdown("---")
if current_streak >= 30: st.success("🏆 أداء مذهل! لديك سلسلة التزام تتجاوز 30 يوماً.")
elif current_streak >= 14: st.success("🔥 رائع! أنت ملتزم منذ أسبوعين متتاليين.")
elif current_streak >= 7: st.success("💪 أحسنت! أكملت أسبوعاً كاملاً من الالتزام.")
elif current_streak >= 3: st.info("👏 استمر، أنت تبني عادة ممتازة.")
else: st.warning("🚀 ابدأ اليوم بالحفاظ على سلسلة الالتزام.")

# ==========================================
# 📥 قسم إدخال البيانات (تم الإصلاح الجذري هنا)
# ==========================================
...
# دالة مساعدة لإنشاء حقول وأزرار الوقت بشكل موحد لمنع تكرار الـ Widgets
def render_duration_section(col_context):
    with col_context:
        # 🟢 [الإصلاح الأهم]: تم إزالة الوسيط المسمى value والاعتماد كلياً على key المربوط بالـ session_state لمنع تعارض الأزرار.
        ...
        b1.button("⏱️ 30 د", on_click=set_duration, args=(0.5,), use_container_width=True)
        b2.button("⏱️ 1 ساعة", on_click=set_duration, args=(1.0,), use_container_width=True)
        b3.button("⏱️ 1.5 س", on_click=set_duration, args=(1.5,), use_container_width=True)
        b4.button("⏱️ 2 ساعتين", on_click=set_duration, args=(2.0,), use_container_width=True)
...
if not df_db.empty:
    ...
    st.download_button(label="📥 تحميل سجل تمارينك كملف Excel منسق ونظيف", data=buffer.getvalue(), file_name="my_gym_activities.xlsx", mime="application/vnd.ms-excel", use_container_width=True)
