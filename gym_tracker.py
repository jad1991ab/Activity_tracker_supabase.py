...
if not df_db_calc.empty:
    ...
    unique_days = sorted(set(df_db_calc["date_only"]))
    streak = 0
    check_day = today
...
achievements.append(("⏱️", "أول 10 ساعات", total_hours >= 10))
achievements.append(("💪", "50 ساعة", total_hours >= 50))
achievements.append(("🚀", "100 ساعة", total_hours >= 100))
...
# ==========================================
    ...
    if current_streak >= 30:
        ...
        st.warning("🚀 حافظ على سلسلة الالتزام وسجّل نشاطك اليوم أولاً بأول!")
    ...
    c4.metric("📋 إجمالي الأنشطة", activities_count)
    ...
    with col_p3:
        st.metric("متوسط إنجازك السابق", f"{avg_previous_hours} ساعة")
